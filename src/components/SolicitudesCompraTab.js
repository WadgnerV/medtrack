...
export default function SolicitudesCompraTab({ profile, inventoryItems }) {
  ...
  async function load() {
    ...
    const { data } = await supabase.from('purchase_orders')
      .select('*, creator:created_by(id, first_name, last_name, role, profile:id(email)), approver:approved_by(first_name, last_name), items:purchase_order_items(*, inventory_item:item_id(name, unit, sku))')
