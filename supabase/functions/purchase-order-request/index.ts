...
serve(async (req) => {
  ...
  const itemsHtml = items.map((i: any) => `
    <tr>
      <td style="padding:10px 12px;border-bottom:1px solid #f0f0f0;font-size:13px;font-weight:500;">${i.name}</td>
      <td style="padding:10px 12px;border-bottom:1px solid #f0f0f0;font-size:13px;text-align:center;">${i.quantity} ${i.unit}</td>
      <td style="padding:10px 12px;border-bottom:1px solid #f0f0f0;font-size:13px;text-align:center;color:${i.en_minimo ? '#D85A30' : '#555'};">
        ${i.stock_actual} ${i.unit}
        ${i.en_minimo ? '<br><span style="font-size:11px;color:#D85A30;font-weight:600;">⚠ En stock mínimo</span>' : ''}
      </td>
    </tr>
  `).join('')
  ...
  const html = `
    ...
    <body style="margin:0;padding:0;background:#f5f5f5;font-family:Inter,Arial,sans-serif;">
      <div style="max-width:580px;margin:40px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
          ...
          <table style="width:100%;border-collapse:collapse;margin-bottom:20px;border:1px solid #eee;border-radius:8px;overflow:hidden;">
            <thead>
              <tr style="background:#f8fbf9;">
                <th style="padding:10px 12px;text-align:left;font-size:11px;color:#888;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;">Ítem</th>
                <th style="padding:10px 12px;text-align:center;font-size:11px;color:#888;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;">Solicitado</th>
                <th style="padding:10px 12px;text-align:center;font-size:11px;color:#888;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;">Stock actual</th>
              </tr>
            </thead>
